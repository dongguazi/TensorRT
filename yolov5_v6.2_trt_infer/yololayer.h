...
namespace Yolo
{
    static constexpr int CHECK_COUNT = 3;//每个location的box数量
    static constexpr float IGNORE_THRESH = 0.1f;//忽略框的置信度阈值
    struct YoloKernel
    {
        int width;
        int height;
        float anchors[CHECK_COUNT * 2];//锚框
    };
    static constexpr int MAX_OUTPUT_BBOX_COUNT = 1000;//最大box数量
    static constexpr int CLASS_NUM = 80;//分类类别数
    static constexpr int INPUT_H = 640;  // yolov5's input height and width must be divisible by 32.
    static constexpr int INPUT_W = 640;

    static constexpr int LOCATIONS = 4;//4个坐标值
    struct alignas(float) Detection {
        //center_x center_y w h
        float bbox[LOCATIONS];//框的坐标【center_x center_y w h】
        float conf;  // 置信度=bbox_conf * cls_conf
        float class_id;//类别ID
    };
}
